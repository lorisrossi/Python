...
def readwordlist(filename):
    ...
    prefixset = set(x for word in wordset for x in prefixes(word))
...
ANY = anchor(LETTERS) # Anchor that can be any letter
...
def removed(letters, remove):
    "Return a str of letters, but with each lettere in 'remove' removed once."
    for L in remove:
...
def add_suffixes(hand, pre, results):
    """Return the set of words that can be formed by extending pre
...
def row_plays(hand, row):
    ...
    for (i, sq) in enumerate(row[1:-1], 1): # remember, row extremes are confines
...
def set_anchors(row, j, board):
    ...
    by cross-words tobe only a subset of letters."""
...
def test():
    ...
    assert (word_plays('ADEQUAT', set('IRE')) == # Regression test
...
#print test()
#print WORDS
#print word_plays('ABCDEFG', set('HIL'))
#show(a_board())
#print all_plays('ABCDEFG', a_board())
